(function ($) {
	...
	var BattleRoom = this.BattleRoom = ConsoleRoom.extend({
		...
		calculateModifiedStats: function (pokemon, pokemonData) {
			var stats = {};
		...
		pokemonHasType: function (pokemon, type, types) {
		...
		updateMoveControls: function (type) {
			...
			if (type === 'movetarget') {
				...
				var pokemonData, pokemon, theirActive = room.battle.yourSide.active[0];
				var isActive = (type === 'pokemon');
				if (room.myPokemon) {
					for (var i = 0; i < room.myPokemon.length; i++) {
						if (!room.myPokemon[i].active) continue;
						pokemonData = room.myPokemon[i];
						pokemon = pokemonData;
						break;
					}
				}
